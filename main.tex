...
\begin{itemize}
  \item Latency does not depend on the duration of the experiment moreover it linearly increases with the number of open connections (see \Cref{fig:latency-to-oc-100}). This is especially true for small file sizes however when the file size grows beyond a certain limit, the number of connections will become irrelevant to the introduced latency (see \Cref{fig:latency-to-oc-1000}). 
...
\Cref{fig:throughput-to-size} shows that, in the case of inter-node communication, an increase in the number of Open Connections will increase the throughput, however, for file sizes above 1 MB, the influence becomes negligible.
\begin{figure}
...
\begin{figure}[H]
    ...
    \end{minipage}
\end{figure}
Similar results are also obtained on Mistral over InfiniBand:
\begin{figure}[H]
    \centering
    \begin{minipage}{0.49\textwidth}
        \centering
